...
TIMESTAMP=$(date +%S)
# output_file_format="${output_dir}/${TIMESTAMP}.pcap"
# output_file_format="${output_dir}/${TIMESTAMP}.pcap" #################################change to dump.pcap
output_file_format="${output_dir}/dump.pcap"
options="-n -nn -N -s 0"
...
# post_rotate_command="${script_dir}"/TCPDUMP_and_CICFlowMeter/convert_pcap_csv.sh ###################################################################Chnage this 
...
sudo tcpdump ${options} -z "${post_rotate_command}" -i ${interface} -c 100 -w "${output_file_format}"
